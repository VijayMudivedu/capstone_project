...
# Coverting the targe variable to factor
...
```
### removing the records filled defective foot notes.
...
cor.plot(safety_of_care_group,stars = FALSE,numbers = TRUE,colors = TRUE,cex = 0.5, show.legend = FALSE,upper = F,
         xlas = 2,cex.axis = 0.6,main = "Patient Safety group measure correlation")

# computing the mean loading
patient_safety_loading <- as.vector(apply(safety_loadings[,1:nfactors],1,FUN = mean))
...
cor_matrix <- cbind(cor_matrix, round(patient_safety_loading, 2))
...
master_df_final_safety_care_measures <- cbind(master_df_final$Provider.ID,master_df_final[,which(colnames(master_df_final) %in% colnames(safety_of_care_group))])
...
# calculating group score of Patient Experience and Saftety of Care
mean_weights
...
plot(density(saftey_group_score$saftey_score))
```

### Generic function that would convert the remaining groups to group scores


- we need to do this for 6 other groups, so
- let's put this entire code in a function
- it takes in the main group measures and the measure correlation as input
...
nfactors = n_factors
scores = "regression"
safety_care_fact_anal <- fa(r = group_measure,rotate = rotate,fm = fm,scores = scores,nfactors = nfactors)
...
# # using sweep function to multiply mean_weights with each hospital group measure to evaluate to the total group score from all the measures.
# master_df_final_group_measure$score <- apply(sweep(master_df_final_group_measure[,-1],
#                                                    MARGIN = 2, mean_weights/length(mean_weights),'*'),1,sum) %>% round(digits = 3)
# 
...
weights <- c(imaging_eff = process_group_weight, 
             timeliness = process_group_weight,
             effectivesss = process_group_weight,
             mortality = out_grp_weight, 
             safety = out_grp_weight, 
             effectiveness = out_grp_weight,
             experience = out_grp_weight, 
             readmission = out_grp_weight, 
             timeliness = out_grp_weight)
...
for (row in 1:nrow(group_scores)) {
  if (sum(is.na(group_scores[row, -1])) > 0) {    # if NAs are found in the group_scores
    ...
    print(sum(weights[-invalid_indices]*reproportioned_weights)) # product the
...
melt(data = final_scores[,-1],variable.name = "measure") %>%
  ...
  facet_wrap(facets = ~measure,scales = "free",ncol = 3) +
...
ggplot(group_scores, aes(x=1, y=final_score)) + geom_boxplot(na.rm=TRUE) + 
  ylab("Final Score") + xlab(NULL) + geom_hline(yintercept= 676, col="red") + ggtitle("Provider Score") + 
  theme(plot.title = element_text(hjust = 0.5))
...
final_scores_merged %>% group_by(new_cluster_id) %>%
  summarise(avg_final_score = mean(final_score, na.rm = T), avg_mort_scr = mean(mort_score, na.rm = T), 
            ...
            avg_img_scr = mean(imaging_eff_score, na.rm=T), avg_exp_scr = mean(patient_exp_score, na.rm=T))
```
...
group_scores_merged <- merge(group_scores, final_scores, by="Provider.ID")
head(final_scores_merged)
...
# melt(data = final_scores[,-1],variable.name = "measure") %>%
#   ggplot(aes(new_cluster_id,value*100)) + geom_boxplot(na.rm = T) + 
#   #geom_hline(aes(yintercept = final_scores_merged[which(final_scores_merged$Provider.ID == 140010),])) +
#   facet_wrap(facets = ~measure,scales = "free",ncol = 3) + 
#   xlab("Star rating") + ylab("Score") + theme_minimal() 
```
...
# Hosptial Safetey - -- Provider Analysis
...
```
 ...
 we  know that within experience, H_COMP_1_LINEAR_SCORE, H_HSP_RATING_LINEAR_SCORE,
...
head(rbind(avg_readm, provider_readm))
```
...
head(rbind(avg_mort, provider_mort))
```
