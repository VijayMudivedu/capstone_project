...
# Distribution of target variable Hospital Rating
```{r}
round(prop.table(table(master_df_final$Hospital.overall.rating)),3)
paste0(round(prop.table(table(master_df_final$Hospital.overall.rating))*100,2),"%")
```

```{r}

zz <- data.frame(master_df[which(master_df$Hospital.overall.rating %in% "Missing"),-c(1,2)] %>% apply(MARGIN = 2,FUN = function(x) sum(is.na(x)))) 
names(zz)[1] <- "count"
zz$measure_id <- row.names(zz)
ggplot(zz,aes(reorder(measure_id,-count),count)) + geom_col(fill = "steelblue") + labs(title = "NAs induced in the Measure.ID's due to missing ratings") +
  theme(axis.text.x = element_text(angle = 90,hjust = 1),axis.title.x = element_blank(), plot.title = element_text(hjust = 0.5,face = "bold"))

```


Which Hospitals have more Not Available Scores across different measures?
- Most of the NAs in the measures is due the Missing rating in the Hosptial Rating.

```{r}
zz_master_df <- master_df_final
zz_master_df$na_sums <- rowSums(is.na(master_df_final[,-c(1,2)]))
zz_master_df$Provider.ID[which(zz_master_df$na_sums == 63)]
```

Different groups identified so far are:
1. Mortality group
2. Saftey of Care
3. Readmission
...
zvar1 <- which(names(master_df_final) %in% zvar2)
safety_of_care_group <- master_df_final[,zvar1]
head(safety_of_care_group)
```
...
### Variables of importance identifeid by the RandomForest
...
vars_predict <- setdiff(x = names(train_df[,-1]),y = "Hospital.overall.rating")
predict_rf_no_na <- stats::predict(object = model_rf_no_na,test_df[vars_predict])
# Confusion Matrix
confusionMatrix(predict_rf_no_na,test_df$Hospital.overall.rating)
```

### model parameters of importance are
```{r}
model_rf_no_na$importance
```