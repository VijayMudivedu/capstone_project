...
safety_care_fact_anal <- fa(r = safety_of_care_group,nfactors = 4,rotate = rotate,fm = fm)
...
patient_exp_factor_anal <- fa(patient_experience_group,nfactors = 3,rotate = "varimax",scores = "regression",fm = "ml",n.iter = 5)
patient_exp_factor_anal$loadings
patient_exp_factor_anal$score.cor
rowMeans(as.data.frame(patient_exp_factor_anal$scores))*rescale(patient_experience_group)
rowMeans(patient_exp_factor_anal$loadings)
...
readmission_group_factor_anal <- fa(readmission_group,nfactors = 2,rotate = "varimax",scores = "regression",fm = "ml")
...
mortality_group_factor_anal <- fa(mortality_group,nfactors = 1,rotate = "varimax",scores = "regression",fm = "ml")
...
```{r}
effectiveness_group_factor_anal <- fa(effectiveness_group,nfactors = 3,rotate = "varimax",scores = "regression",fm = "ml")
...
timeliness_group_factor_anal <- fa(timeliness_group,nfactors = 2,rotate = "varimax",scores = "regression",fm = "ml")
...
```{r}
# First five factors that explain the maximum variance between the groups
imaging_eff_group_factor_anal <- fa(medical_imaging_efficiency_group[,-1],nfactors = 2,rotate = "varimax",scores = "regression",fm = "ml")
...
pairs.panels(final_scores[,-c(1,9)])
# doesn't indicate strong correlation between the groups.
...
# initializing the r_squared vector
...
final_scores %>% group_by(rating_clster) %>% summarise(count_rws = n()) %>% mutate(total_rows = sum(count_rws),count_rws*100/total_rows)
...
# comparing the hospial ratings of random forest and clustering  
confusionMatrix(data = factor(predict_master_df$Hospital.overall.rating),reference = factor(final_scores$rating_clster))
...
# library(lavaan)
# 
# cms.rating.model <- ' zsafe =~ COMP_HIP_KNEE + PSI_90_SAFETY + HAI_1_SIR + HAI_2_SIR + HAI_3_SIR + HAI_4_SIR + HAI_5_SIR + HAI_6_SIR
# zpatient =~ H_CLEAN + H_COMP_1 + H_COMP_2 + H_COMP_3 + H_COMP_4 + H_COMP_5 + H_COMP_6 + H_COMP_7 + H_HSP_RATING + H_QUIET + H_RECMND
# zread =~ READM_30_AMI + READM_30_CABG + READM_30_COPD + READM_30_HF + READM_30_HIP_KNEE + READM_30_HOSP_WIDE + READM_30_PN + READM_30_STK
# zmort =~ MORT_30_AMI + MORT_30_CABG + MORT_30_COPD + MORT_30_HF + MORT_30_PN + MORT_30_STK + PSI_4_SURG_COMP
# zimag =~ OP_10 + OP_11 + OP_13 + OP_14 + OP_8
# ztim =~ ED_1b + ED_2b + OP_18b + OP_20 + OP_21 + OP_3b + OP_5
# zeff =~ CAC_3 + IMM_2 + IMM_3_OP_27 + OP_22 + OP_23 + OP_29 + OP_30 + OP_4 + PC_01 + STK_1 + STK_4 + STK_6 + STK_8 + VTE_1 + VTE_2 + VTE_3 + VTE_5 + VTE_6 '
# 
# 
# # model fit
# cms_fit <- lavaan::cfa(model = cms.rating.model, data = master_df_imputed_final[,-1])
# cms_fit <- lavaan::sem(model = cms.rating.model, data = master_df_imputed_final[,-1])
# summary(cms_fit)
# lavInspect(cms_fit,"optim.gradient")
# paste(zvar_mort[-1],collapse = " + ")
# 
...
master_df_final[which(master_df$Provider.ID %in% "140010"),]

final_scores[which(final_scores$rating_clster %in% "4"),]
