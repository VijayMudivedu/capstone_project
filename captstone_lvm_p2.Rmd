...
safety_of_care_group <- master_df_imputed_final[,zvar1]
head(safety_of_care_group)
```
...
zvar1 <- which(names(master_df_imputed_final) %in% zvar2)
patient_experience_group <- master_df_imputed_final[,zvar1]
head(patient_experience_group)
```

## 3 Effectiveness Group
## Latent Variable model
```{r}
# safety of care
...
zvar1 <- which(names(master_df_imputed_final) %in% zvar2)
effectiveness_group <- master_df_imputed_final[,zvar1]
head(effectiveness_group)
```

## 4 timeliness of care Group
## Latent Variable model
```{r}
# safety of care
zvar2 <- c("Provider.ID","ED_1b","ED_2b","OP_18b","OP_20","OP_21","OP_3b","OP_5")
zvar1 <- which(names(master_df_imputed_final) %in% zvar2)
timeliness_of_care_group <- master_df_imputed_final[,zvar1]
head(timeliness_of_care_group)
```

## 5 medical imagine efficiency group
## Latent Variable model
```{r}
names(master_df_imputed_final)
# safety of care
zvar2 <- c("Provider.ID","OP_10","OP_11","OP_13","OP_14","OP_8")
zvar1 <- which(names(master_df_imputed_final) %in% zvar2)
medical_imaging_efficiency_group <- master_df_imputed_final[,zvar1]
head(medical_imaging_efficiency_group)
```
...
zvar1 <- which(names(master_df_imputed_final) %in% zvar2)
readmission_group <- master_df_imputed_final[,zvar1]
head(readmission_group)
...
zvar1 <- which(names(master_df_imputed_final) %in% zvar2)
mortality_group <- master_df_imputed_final[,zvar1]
head(mortality_group)
```

# Step 1: Standarizing the variable each of the groups
...
Standardizing the 
```{r}
safety_of_care_group_std <- apply(safety_of_care_group[,-1],MARGIN = 2,FUN = function(x) round(standardization(x),4))
factanal(safety_of_care_group_std,factors = 3)

apply(safety_of_care_group[,-1],MARGIN = 2,pnorm)
